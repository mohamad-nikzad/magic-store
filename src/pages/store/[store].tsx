...
const Store: NextPage = () => {
  ...
  const store: string = router.query.store
    ? Array.isArray(router.query.store)
      ? ""
      : router.query.store
    : "";
  const user = trpc.useQuery(["get-user", store], { enabled: false });
  useEffect(() => {
    !user.isFetched && user.refetch();
  }, [user]);
    ...
    <div className="w-full font-mono  relative min-h-screen from-base-100 to-base-300 bg-gradient-to-br">
      <div className="navbar bg-base-300 shadow top-0 sticky z-10">
        <a className="btn btn-ghost normal-case text-xl">
          {user.data?.user?.instagram_id || router.query?.store}
        </a>
      </div>
